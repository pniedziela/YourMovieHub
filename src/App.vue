        ...
        <div class="row" style="margin:5px; float: left; border:5px solid;">
          ...
          <div class="column">
            <v-dialog v-model="dialog" width="600px">
              <template v-slot:activator="{ on }">
              <img style="width: 100%; height: auto" v-on="on" v-bind:src="moviesList.Search[0].Poster">
              </template>
              <v-card>
                <v-card-title>
                  <span class="headline">This Movie details</span>
                </v-card-title>
                <v-card-text>{{moviesList.Search[0].Plot}}</v-card-text>
                <v-card-actions>
                  <v-spacer></v-spacer>
                  <v-btn color="green darken-1" text @click="dialog = false">Super</v-btn>
                  <v-btn color="green darken-1" text @click="dialog = false">Jeszcze lepiej</v-btn>
                </v-card-actions>
              </v-card>
            </v-dialog>

              <div class="title white--text"  style="padding: 15px; text-align: center">{{moviesList.Search[0].Plot}}</div>
          </div>
          <div class="column">
